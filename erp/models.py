...
class Base(object):
    ...
    def get_session(self):
        self.__session__ = self.__session__ or DBSession()
...
class Quotation(Base, Audited):
    ...
    @property
    def __acl__(self):
        access_list = super().__acl__
        if not self.current:
            access_list.remove((Allow, self.owner.username, 'EDIT'))
        return access_list
...
class ViewFactory(object):
    ...
    def __acl__(self):
        ...
        if self.__view__ and hasattr(self.__view__, '__permissions__'):
            view_acl = list(self.__view__.__permissions__)
            view_acl.extend([a for a in access_list if a not in view_acl])
            access_list = view_acl
